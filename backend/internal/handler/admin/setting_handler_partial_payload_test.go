...
func TestUpdateSettingsPartialPayloadKeepsUnsentKeys(t *testing.T) {
	h, repo := newStepUpSwitchTestHandler(t, map[string]string{
		...
		service.SettingKeyOpenAIFastPolicySettings: `{"rules":[{"enabled":true,"match":{"models":["gpt-5.3-codex"]},"action":{"inject_service_tier":"priority"}}]}`,
	})
	...
	require.JSONEq(t,
		`{"rules":[{"enabled":true,"match":{"models":["gpt-5.3-codex"]},"action":{"inject_service_tier":"priority"}}]}`,
