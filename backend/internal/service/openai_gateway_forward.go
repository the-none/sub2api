...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if account.UsesOpenAICodexProtocol() {
		...
		// 每次账号尝试都覆盖 context；nil 会清除上一次 failover 账号留下的指纹。
	...
	} else if !requestView.ServiceTierPresent {
		// Missing-tier injection has already evaluated the global policy using
		// tierPresent=false. Do not evaluate it a second time as an explicit
		// priority tier, because that could change first-match semantics.
		if s.shouldInjectOpenAIFastPriority(ctx, account, upstreamModel) {
			markPatchSet("service_tier", OpenAIFastTierPriority)
		}
	}
	if rawTier != "" {
