...
func TestHandleUpstreamError_AnthropicFableCreditsRequiredOnlyMarksModelRateLimit(t *testing.T) {
	...
	weeklyReset := time.Now().Add(80 * time.Hour).Truncate(time.Second)
	headers := http.Header{}
	...
	headers.Set("anthropic-ratelimit-unified-7d_oi-utilization", "0.81")
	headers.Set("anthropic-ratelimit-unified-7d_oi-reset", strconv.FormatInt(weeklyReset.Unix(), 10))
	...
	require.Equal(t, 0.81, repo.lastExtraUpdates["passive_usage_7d_oi_utilization"])
	require.Equal(t, weeklyReset.Unix(), repo.lastExtraUpdates["passive_usage_7d_oi_reset"])
}
