		...
		Options: openaiwsv2.RelayOptions{
			...
			OnTurnComplete: func(turn openaiwsv2.RelayTurnResult) {
				...
				turnResult := &OpenAIForwardResult{
					RequestID: openAIWSTurnRequestID(turn.RequestID, turnRequestIDBase, turnNo),
					Usage: OpenAIUsage{
...
func openAIWSTurnRequestID(upstreamRequestID, fallbackBase string, turn int) string {
	...
	return fmt.Sprintf("%s:ws-turn:%d", base, max(turn, 1))
}

func assignOpenAIWSTurnRequestID(result *OpenAIForwardResult, fallbackBase string, turn int) {
	if result == nil {
		return
	}
	result.RequestID = openAIWSTurnRequestID(result.RequestID, fallbackBase, turn)
}
