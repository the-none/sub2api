...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	turnRequestIDBase := resolveUsageBillingRequestID(ctx, "")
	...
	if forceHTTPBridge || s.shouldBridgeOpenAIWSHTTP(account, firstPayload.payloadBytes, firstPayload.previousResponseID) {
		...
		for turn := 1; ; turn++ {
			...
			upstreamResponseID := ""
			if result != nil {
				upstreamResponseID = strings.TrimSpace(result.RequestID)
			}
			assignOpenAIWSTurnRequestID(result, turnRequestIDBase, turn)
			...
			if upstreamResponseID != "" && stateStore != nil {
				ttl := s.openAIWSResponseStickyTTL()
				logOpenAIWSBindResponseAccountWarn(groupID, account.ID, upstreamResponseID, stateStore.BindResponseAccount(ctx, groupID, upstreamResponseID, account.ID, ttl))
			}
	...
	for {
		...
		lastTurnResponseID = upstreamResponseID
		...
		if upstreamResponseID != "" && stateStore != nil {
			ttl := s.openAIWSResponseStickyTTL()
			logOpenAIWSBindResponseAccountWarn(groupID, account.ID, upstreamResponseID, stateStore.BindResponseAccount(ctx, groupID, upstreamResponseID, account.ID, ttl))
			stateStore.BindResponseConn(upstreamResponseID, connID, ttl)
		}
