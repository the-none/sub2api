...
func TestEmitTurnCompleteCoverage(t *testing.T) {
	...
	// 缺少 response_id 的 terminal 事件仍需推进轮次，避免后续 turn 编号错位和漏计用量。
	var missingIDTurn RelayTurnResult
	emitTurnComplete(func(turn RelayTurnResult) {
		called++
		missingIDTurn = turn
	}, &relayState{requestModel: "gpt-5"}, observedUpstreamEvent{
	...
	require.Empty(t, missingIDTurn.RequestID)
	require.Equal(t, "gpt-5", missingIDTurn.RequestModel)
...
func TestEmitTurnCompleteKeepsMultiTurnSequenceWhenLaterTerminalLacksResponseID(t *testing.T) {
	t.Parallel()

	state := &relayState{requestModel: "gpt-5"}
	turns := make([]RelayTurnResult, 0, 2)
	record := func(turn RelayTurnResult) {
		turns = append(turns, turn)
	}

	emitTurnComplete(record, state, observedUpstreamEvent{
		terminal:   true,
		eventType:  "response.completed",
	...
	require.Equal(t, "resp_first", turns[0].RequestID)
	require.Empty(t, turns[1].RequestID)
	require.Equal(t, Usage{InputTokens: 5, OutputTokens: 4}, turns[1].Usage)
}

func TestFinalizeTerminalWithoutActiveTimingPreservesAggregateMetadata(t *testing.T) {
	t.Parallel()

	state := &relayState{
		lastResponseModel:       "gpt-5.4",
		lastResponseServiceTier: "priority",
		responseConflict:        true,
	}
	observed := finalizeObservedRelayTerminal(state, observedUpstreamEvent{
		eventType: "response.completed",
	}, time.Now())

	require.True(t, observed.terminal)
	require.Empty(t, observed.responseID)
	require.Equal(t, "gpt-5.4", state.lastResponseModel)
	require.Equal(t, "priority", state.lastResponseServiceTier)
	require.True(t, state.responseConflict)
}
