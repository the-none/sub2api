...
func TestRelay_OnTurnComplete_MissingTerminalResponseIDPreservesTurnMetadata(t *testing.T) {
	t.Parallel()
	...
	upstreamConn := newPassthroughTestFrameConn([]passthroughTestFrame{
		...
		{
			msgType: coderws.MessageText,
			payload: []byte(`{"type":"response.created","response":{"id":"resp_turn_2","model":"gpt-5.4"}}`),
		},
		{
			msgType: coderws.MessageText,
			payload: []byte(`{"type":"response.output_text.delta","delta":"done"}`),
		},
		{
			msgType: coderws.MessageText,
			payload: []byte(`{"type":"response.completed","response":{"model":"gpt-5.4","service_tier":"priority","usage":{"input_tokens":3,"output_tokens":4}}}`),
		},
	}, true)
	...
	defer cancel()
	turns := make([]RelayTurnResult, 0, 2)
	result, relayExit := Relay(ctx, clientConn, upstreamConn, []byte(`{"type":"response.create","model":"gpt-5.4","input":[]}`), RelayOptions{
	...
	require.Equal(t, "gpt-5.4", turns[1].ResponseModel)
	require.Equal(t, "priority", turns[1].ResponseServiceTier)
	require.False(t, turns[1].StartedAt.IsZero())
	require.NotNil(t, turns[1].FirstTokenMs)
	require.GreaterOrEqual(t, turns[1].Duration, time.Duration(0))
	require.Equal(t, "gpt-5.4", result.ResponseModel)
	require.Equal(t, "priority", result.ResponseServiceTier)
