...
func TestOpenAI429FastPath_RetriesOAuthWhenQuotaHeadersAreNotExhausted(t *testing.T) {
	repo := &oauth429RateLimitRepo{}
	rateLimits := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
	svc := &OpenAIGatewayService{rateLimitService: rateLimits}
	rateLimits.SetAccountRuntimeBlocker(svc)
	...
	require.Nil(t, resetAt, "non-exhausted quota headers must not cause a week-long cooldown")
	require.False(t, shouldDisable)
	require.True(t, svc.ShouldRetryOpenAIOAuth429(account, headers, nil))
	require.False(t, svc.isOpenAIAccountRuntimeBlocked(account))
	require.Zero(t, repo.setRateLimitedCalls)
	...
	require.Positive(t, time.Until(blockedUntil))
	require.Less(t, time.Until(blockedUntil), time.Minute, "exhausted retry budget uses fallback, not the seven-day window")
}
