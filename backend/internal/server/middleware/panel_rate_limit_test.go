...
func TestPanelRateLimiterPublicIPIgnoresUntrustedForwardedHeaders(t *testing.T) {
	allower := &fakePanelAllower{}
	p := &PanelRateLimiter{
		limiter:        allower,
		settingService: newPanelRateLimitTestService(t, `{"enabled":true,"user_rpm":0,"heavy_rpm":0,"exempt_admin":true,"public_ip_rpm":1}`),
	...
	// 私网 XFF 不能关闭限流，轮换公网 XFF 也不能切换桶。
	require.Equal(t, http.StatusOK, send("10.0.0.7"))
	require.Equal(t, http.StatusTooManyRequests, send("198.51.100.8"))

	allower.mu.Lock()
	defer allower.mu.Unlock()
	require.Contains(t, allower.counts, "panel:public:ip:203.0.113.9")
	require.NotContains(t, allower.counts, "panel:public:ip:10.0.0.7")
