...
func TestRateLimiterIgnoresUntrustedForwardedIPSnapshot(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	// 即使兼容模式允许其他路径读取原始转发头，限流键也不能受其影响。
	...
	// 未配置可信代理时，伪造不同 XFF 仍归入同一个直连地址限流桶。
	require.Equal(t, http.StatusOK, send("198.51.100.1"))
	require.Equal(t, http.StatusTooManyRequests, send("198.51.100.2"))

	require.Contains(t, callCounts, "rate_limit:fwd:127.0.0.1")
	require.NotContains(t, callCounts, "rate_limit:fwd:198.51.100.1")
	require.NotContains(t, callCounts, "rate_limit:fwd:198.51.100.2")
}

func TestRateLimiterHonorsConfiguredTrustedProxy(t *testing.T) {
	gin.SetMode(gin.TestMode)
