...
- `security.trust_forwarded_ip_for_api_key_acl` enables legacy raw forwarded-header takeover for API-key ACL/session binding (enabled by default for upgrade compatibility); security-sensitive IP rate limits always enforce `server.trusted_proxies`
...
Every reverse-proxy deployment must configure `server.trusted_proxies` with the exact CIDRs that connect directly to Sub2API. Authentication, invalid-credential, and public-panel IP rate limits never trust raw forwarded headers; without this proxy chain, all clients behind a proxy can share the proxy's rate-limit bucket.
