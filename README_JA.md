...
- `security.trust_forwarded_ip_for_api_key_acl` - API キー ACL/セッションバインド向けの従来の生転送ヘッダー上書きを制御（アップグレード互換性のため既定で有効）。セキュリティ上重要な IP レート制限は常に `server.trusted_proxies` を厳格に使用
...
すべてのリバースプロキシ構成で、Sub2API に直接接続するプロキシの正確な CIDR を `server.trusted_proxies` に設定する必要があります。認証、無効な資格情報、公開パネルなどのセキュリティ上重要な IP レート制限は生の転送ヘッダーを信頼しないため、未設定ではプロキシ配下の全クライアントが同じレート制限バケットを共有します。
