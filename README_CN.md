...
- `security.trust_forwarded_ip_for_api_key_acl` 控制 API Key ACL/会话绑定的旧版原始转发头接管（为升级兼容默认开启）；安全敏感的 IP 限流始终严格使用 `server.trusted_proxies`
...
任何反向代理部署都必须在 `server.trusted_proxies` 中配置直接连接 Sub2API 的精确代理 CIDR。登录、无效凭证和公开面板等安全敏感 IP 限流绝不会信任原始转发头；未配置可信代理链时，代理后的所有客户端会共用代理地址对应的限流桶。
